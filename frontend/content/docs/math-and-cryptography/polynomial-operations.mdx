...
This document defines the API for working with polynomials over BN254's scalar field `Fr`, using the field arithmetic primitives from `soroban-zk-core`.
...
```rust
use soroban_zk_core::{Bn254, Fr, SafeFrom};
use ethnum::u256;
...
## Integration with `soroban-zk-core`

All polynomial arithmetic uses `soroban-zk-core`'s BN254 field operations:
